...
local key_prefix_isdeny = "ngx:uidfreq:isdeny:"
local key_prefix_stime = "ngx:uidfreq:stime:"
local key_prefix_count = "ngx:uidfreq:count:"
local key_prefix_uid = "ngx:uid:"
...
function _M.check_uid_freq(self)
    ...
    -- ngx.log(ngx.ERR,"testlog1 ","uid: "..uid.." : nid : "..nid)
    if nid <= 0 and uid <= 0 then
        return error_status
    elseif nid > 0 and uid <= 0 then
        return 1
    end

    local rds_key = uid
    -- ngx.log(ngx.ERR,"testlog4 ","uid: "..uid.." : nid : "..nid)
    -- 连接redis
